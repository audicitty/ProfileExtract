...
5. **Job match scoring is circular — the resume is scored against itself.** [src/lib/jobs.ts:93](src/lib/jobs.ts#L93) — `inferSkillsFromTitle` seeds each job's `skills_required` with the candidate's own skills (`candidateSkills.slice(0, 4).forEach(...)`). `scoreJobsWithResume` then scores the candidate on how many of those they match, so up to four matches are guaranteed by construction and `match_score` is inflated for every job regardless of actual fit. Fix: remove the candidate-skill seeding entirely.
6. **Job descriptions and required skills are fabricated, not scraped.** [src/lib/jobs.ts:240](src/lib/jobs.ts#L240) — `description` is a hardcoded template string (`"Active job opening at {company} in {location}..."`) and `skills_required` comes from `inferSkillsFromTitle`'s hardcoded keyword buckets. The LinkedIn guest **search** endpoint only returns title/company/location, so no real requirements text enters the system and all matching runs on guesses. Fix: fetch each posting's body from the per-job guest endpoint (`jobs-guest/jobs/api/jobPosting/{id}` — needs verifying it still works and isn't rate-limited), keeping `inferSkillsFromTitle` only as a fallback. See `idea.md` §10.2.
7. **Match score is floored at 62 and skill matching collides on substrings.** [src/lib/jobs.ts:433](src/lib/jobs.ts#L433) — `62 + skillRatio * 33` compresses every score into 62–95, and an empty `jobSkills` defaults `skillRatio` to `0.7` rather than scoring low. Separately [src/lib/jobs.ts:421](src/lib/jobs.ts#L421) matches via bidirectional substring containment (`cs.includes(skillLower) || skillLower.includes(cs)`), so "Java" matches "JavaScript" and "R" matches "React". Fix: use the full 0–100 range and match on token boundaries. Depends on #6 — real skills data has to exist before the score means anything.
